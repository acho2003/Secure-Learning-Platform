...
import { getAnnouncements, createAnnouncement, getResources, uploadResource, downloadResourceFile } from '../services/apiService';
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
          ...
          <form onSubmit={handleCreateAnnouncement} className="mb-6 space-y-4">
            ...
            <div>
              <label htmlFor="ann-title" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Title</label>
              <input id="ann-title" type="text" value={newAnnouncement.title} onChange={(e) => setNewAnnouncement({ ...newAnnouncement, title: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm dark:bg-gray-700" placeholder="Important Update" />
            </div>
            <div>
              <label htmlFor="ann-content" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Content</label>
              <textarea id="ann-content" value={newAnnouncement.content} onChange={(e) => setNewAnnouncement({ ...newAnnouncement, content: e.target.value })} rows={3} className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm dark:bg-gray-700" placeholder="Details about the announcement..." />
            </div>
            <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Post Announcement</button>
          </form>
        ...
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
          ...
          <form onSubmit={handleUploadResource} className="mb-6 space-y-4">
            ...
            <div>
                <label htmlFor="file-upload-input" className="block text-sm font-medium text-gray-700 dark:text-gray-300">File</label>
                <input id="file-upload-input" type="file" onChange={(e) => setFileToUpload(e.target.files ? e.target.files[0] : null)} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"/>
            </div>
            <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Upload</button>
          </form>
          <ul className="space-y-2">
            {isLoading ? <p>Loading...</p> : resources.map(res => (
